const splitIntoSentences = (text) => {
  ...
  // Guard: inner must not start with space (avoids false matches like "…  "Next sentence")
  ...
  const splitSegment = (seg) => {
    ...
    while (i < seg.length) {
      ...
      if (!inQuote && /[.!?…]/.test(ch)) {
        // Collect the full punctuation run
        let punct = '';
        ...
        if (spaceMatch) {
          ...
          // Split if followed by end-of-segment, uppercase, or opening quote/dash
          ...
          // No space but next char is uppercase — split (e.g. "nạn.Sau")
          results.push((current + punct).trim());
          current = '';
          continue;
        }
  ...
  return segments
    ...
    .map(s => {
      ...
      // Remove a stray trailing quote if the segment doesn't start with one
...
const tests = [
  ...
  {
    label: 'Test 6',
    input: 'Trung niên nam tử vừa nói xong, không có gì ngoài ý muốn, đám người trên quảng trường lại nổi lên trận trận châm chọc tao động "Ba đoạn? Hắc hắc, quả nhiên không ngoài dự đoán của ta, "Thiên tài" này một năm rồi vẫn dậm chân tại chỗ a!"',
    expected: [
